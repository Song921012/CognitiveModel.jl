...
ann1 = Flux.Chain(Flux.Dense(1, 64, relu), Flux.Dense(64, 1))
ann2 = Flux.Chain(Flux.Dense(1, 64, relu), Flux.Dense(64, 1))
...
function SVEIR_nn(du, u, p, t)
    ...
    σ1 = 0.19f0
    γ1 = 0.1f0
    β = min(5.0f0, abs(re1(p[1:length(p1)])([t])[1]))
    ν = abs(re2(p[(length(p1)+1):end])([t])[1])
    du[1] = -β * I * S / N - ν * S
